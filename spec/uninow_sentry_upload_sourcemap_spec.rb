  ...
  describe Fastlane::FastFile do
    describe "upload_sourcemap" do
      ...
      it "does not require dist to be specified" do
        ...
        expect(Fastlane::Helper::UninowSentryHelper).to receive(:call_sentry_cli).with(["sentry-cli", "releases", "files", "app.idf-1.0", "upload-sourcemaps", "1.map", "--no-rewrite"]).and_return(true)
      ...
      it "accepts app_identifier" do
        ...
        expect(Fastlane::Helper::UninowSentryConfig).to receive(:parse_api_params).and_return(true)
        expect(Fastlane::Helper::UninowSentryHelper).to receive(:call_sentry_cli).with(["sentry-cli", "releases", "files", "app.idf-1.0", "upload-sourcemaps", "1.map", "--no-rewrite", "--dist", "dem"]).and_return(true)
      ...
      it "does not prepend app_identifier if not specified" do
        expect(Fastlane::Helper::UninowSentryHelper).to receive(:check_sentry_cli!).and_return(true)
        expect(Fastlane::Helper::UninowSentryConfig).to receive(:parse_api_params).and_return(true)
        expect(Fastlane::Helper::UninowSentryHelper).to receive(:call_sentry_cli).with(["sentry-cli", "releases", "files", "1.0", "upload-sourcemaps", "1.map", "--no-rewrite", "--dist", "dem"]).and_return(true)
      ...
      it "default --no-rewrite is omitted when 'rewrite' is specified" do
        expect(Fastlane::Helper::UninowSentryHelper).to receive(:check_sentry_cli!).and_return(true)
        expect(Fastlane::Helper::UninowSentryConfig).to receive(:parse_api_params).and_return(true)
        expect(Fastlane::Helper::UninowSentryHelper).to receive(:call_sentry_cli).with(["sentry-cli", "releases", "files", "1.0", "upload-sourcemaps", "1.map", "--rewrite", "--dist", "dem"]).and_return(true)
